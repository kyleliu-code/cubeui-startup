...
<script>
  import {
    ...
    LIVE,
    CONCERN
  } from '../common/config/tabs.js';
  import list from '../common/data/match-list.js';
  ...
  export default {
    ...
    methods: {
      ...
      subscribe() {
        ...
        this.subscribeDialog.show(); // 这里怎么绑定
      },
      onPullingDown() {
        this.loadMatch('down');
      },
      ...
      loadMatch(type) { // 数据请求
        setTimeout(() => {
          if (Math.random() > 0.5) {
            ...
            if (type === DOWN) {
              this.matchList.unshift(...match); // 随机添加数据
            } else if (type === UP) {
